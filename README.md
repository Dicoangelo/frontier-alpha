...
**Upgrade path:** [Polygon Starter — $29/mo, 100 requests/min](https://polygon.io/pricing). No code change required; the rate-limit headroom alone fixes it. The env var stays `POLYGON_API_KEY`. Rotate the key with `printf "%s" "$NEW_KEY" | vercel env add POLYGON_API_KEY production --force` (the `printf` part matters — `echo` truncates the trailing char; see `~/.claude/memory/feedback_polygon_key_truncation.md`).
