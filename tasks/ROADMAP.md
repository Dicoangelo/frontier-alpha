...
Source: `DESIGN-BRIEF.md` (Apr 19). Four long-running PRDs that ralph-tui / an operator can chew through in order. Each story has explicit acceptance criteria and a dependsOn graph so work parallelizes cleanly.
...
# Or the AI agent agent loop
for prd in tasks/prd-*.json; do
  agent --task "Execute next pending story in $prd, run verification, mark done"
done
```
