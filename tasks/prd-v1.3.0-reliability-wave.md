...
**Description:** As an AI agent or human contributor opening the repo for the first time after the v1.2.x wave, I want `ARCHITECTURE.md`, `CLAUDE.md`, and a new `arch.json` fingerprint to accurately describe what's on disk so my mental model matches reality before I touch code — AND I want CI to fail when those docs drift again.
...
**Files:** `ARCHITECTURE.md`, `CLAUDE.md`, `schemas/arch.json` (new), `scripts/arch-scanner.mjs` (new)
...
- [ ] Reconcile CLAUDE.md "Current State" against `package.json` version + `git log --oneline -20`
- [ ] Re-list `client/src/pages/*.tsx` against `App.tsx` route declarations — every Page has a Route, every Route has a Page
- [ ] Verify Supabase migration count matches CLAUDE.md
...
- [ ] Add `## Operations` section to CLAUDE.md covering: daily checks (5-min `/health/integrations` + smoke status ritual), weekly checks (env audit, error digest, deploy log), incident playbook (when /health says X is degraded → diagnostic ladder), upgrade triggers (link to `feedback_polygon_key_truncation.md` + `project_frontier_alpha_provider_tiers.md`)
...
- [ ] Document the choice + DSN status in CLAUDE.md operations section
- **Weekly digest:**
...
- `CLAUDE.md` (operations section addition for rotation runbook)
...
- **Operations runbook:**
- [ ] CLAUDE.md operations section adds rotation runbook: "key rotated → update Polygon dashboard → run `node scripts/audit-env.mjs --update KEY=NEW_VALUE` → script handles printf + Vercel env add + Railway mirror"
...
- **FR-9:** ARCHITECTURE.md and CLAUDE.md must accurately reflect the on-disk codebase. `arch.json` machine-checkable fingerprint detects future drift.
- **FR-10:** CLAUDE.md must include an Operations section: daily rituals, weekly checks, incident playbook, rotation runbook, upgrade triggers.
...
- **1 operations runbook** in CLAUDE.md covering daily/weekly checks + incident playbook + rotation runbook + upgrade triggers
