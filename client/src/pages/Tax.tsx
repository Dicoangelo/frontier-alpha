    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <MetricCard
          ...
          color={data.totalRealizedGain >= 0 ? 'text-green-600' : 'text-red-600'}
          ...
          <table className="w-full text-sm min-w-[480px]">
            ...
            <tbody>
              <tr className="border-b border-[var(--color-border)]">
                ...
                <td className="px-4 py-3 text-right font-mono text-green-600">{formatCurrency(data.shortTermGains)}</td>
                <td className="px-4 py-3 text-right font-mono text-red-600">{formatCurrency(data.shortTermLosses)}</td>
                <td className={`px-4 py-3 text-right font-mono font-bold ${data.netShortTerm >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                  {formatCurrency(data.netShortTerm, true)}
                </td>
              </tr>
              <tr className="border-b border-[var(--color-border)]">
                <td className="px-4 py-3 font-medium text-[var(--color-text)]">Long-Term</td>
                <td className="px-4 py-3 text-right font-mono text-green-600">{formatCurrency(data.longTermGains)}</td>
                <td className="px-4 py-3 text-right font-mono text-red-600">{formatCurrency(data.longTermLosses)}</td>
                <td className={`px-4 py-3 text-right font-mono font-bold ${data.netLongTerm >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                  {formatCurrency(data.netLongTerm, true)}
                </td>
              </tr>
              <tr>
                <td className="px-4 py-3 font-bold text-[var(--color-text)]">Total</td>
                <td className="px-4 py-3 text-right font-mono font-bold text-green-600">{formatCurrency(data.shortTermGains + data.longTermGains)}</td>
                <td className="px-4 py-3 text-right font-mono font-bold text-red-600">{formatCurrency(data.shortTermLosses + data.longTermLosses)}</td>
                <td className={`px-4 py-3 text-right font-mono font-bold text-lg ${data.totalRealizedGain >= 0 ? 'text-green-600' : 'text-red-600'}`}>
...
function HarvestSection({ opportunities }: { opportunities: HarvestOpportunity[] }) {
  ...
  return (
    <div className="space-y-4">
      {/* Summary banner */}
      <div className="p-4 bg-indigo-500/10 border border-indigo-500/20 rounded-lg flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-sm text-[var(--color-text-muted)]">Total estimated savings: <span className="font-bold text-indigo-600 font-mono">{formatCurrency(totalSavings)}</span></p>
          </div>
        </div>
      </div>
              ...
              {opportunities.map((opp) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-right font-mono text-red-600 font-bold">{formatCurrency(opp.unrealizedLoss, true)}</td>
                    <td className="px-4 py-3 text-right font-mono text-indigo-600 font-bold">{formatCurrency(opp.estimatedTaxSavings)}</td>
                    <td className="px-4 py-3">
                      <div className="flex flex-wrap gap-1">
                        {opp.replacements.map((sym) => (
                          <span key={sym} className="px-2 py-0.5 text-xs font-mono bg-[var(--color-bg-secondary)] text-[var(--color-text-muted)] rounded border border-[var(--color-border)]">
                            {sym}
                          </span>
                        ))}
                      </div>
    ...
    <div className="space-y-4">
      ...
      <div className="p-4 bg-orange-500/10 border border-orange-500/20 rounded-lg">
        <div className="flex items-start gap-3">
          <Info className="w-5 h-5 text-orange-600 flex-shrink-0 mt-0.5" aria-hidden="true" />
          <div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 px-1">
        <p className="text-sm text-[var(--color-text-muted)]">
          ...
          Total disallowed: <span className="font-bold font-mono text-orange-600">{formatCurrency(totalDisallowed)}</span>
        </p>
      </div>
              ...
              >
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-sm text-[var(--color-text-muted)]">
                      Sold {v.saleShares} shares at <span className="text-red-600 font-mono">{formatCurrency(v.saleLoss)}</span> loss &middot;
                      Disallowed: <span className="text-orange-600 font-mono font-bold">{formatCurrency(v.disallowedLoss)}</span>
                    </p>
                  </div>
                </div>
                {isExpanded
                ...
                <div className="mt-4 pt-4 border-t border-[var(--color-border)] text-sm space-y-2">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div>
                      ...
                      <p className="text-red-600 font-mono">Loss: {formatCurrency(v.saleLoss)}</p>
                    </div>
                    <div>
                      ...
                      <p className="text-orange-600 font-mono">Disallowed: {formatCurrency(v.disallowedLoss)}</p>
                    </div>
                  </div>
          ...
          <table className="w-full text-sm min-w-[700px]">
            ...
            <tbody>
              {visibleRows.map((row, idx) => (
                <tr key={idx} className="border-b border-[var(--color-border)] last:border-0 hover:bg-[var(--color-bg-secondary)] transition-colors">
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      <span className="text-[var(--color-text)]">{row.description}</span>
                      <span className={`px-1.5 py-0.5 text-[10px] font-medium rounded ${
                        row.isShortTerm ? 'bg-amber-500/10 text-amber-600' : 'bg-green-500/10 text-green-600'
                      }`}>
                        {row.isShortTerm ? 'ST' : 'LT'}
                      </span>
                      {row.adjustmentCode === 'W' && (
                        <span className="px-1.5 py-0.5 text-[10px] font-medium rounded bg-orange-500/10 text-orange-600">W</span>
                      )}
                    </div>
                  </td>
                  ...
                  <td className="px-4 py-3 text-center font-mono">
                    {row.adjustmentAmount > 0 ? (
                      <span className="text-orange-600">{formatCurrency(row.adjustmentAmount)}</span>
                    ) : (
                      <span className="text-[var(--color-text-muted)]">—</span>
                    )}
                  </td>
                  <td className={`px-4 py-3 text-right font-mono font-bold ${
                    row.gainOrLoss > 0 ? 'text-green-600' : row.gainOrLoss < 0 ? 'text-red-600' : 'text-[var(--color-text-muted)]'
                  }`}>
...
export function Tax() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Tab Selector */}
      <div className="flex gap-1 p-1 bg-[var(--color-bg-secondary)] rounded-lg w-fit overflow-x-auto" role="tablist" aria-label="Tax dashboard tabs">
        {tabs.map((tab) => {
      ...
      {/* Tab Content */}
      <div id={`tabpanel-tax-${activeTab}`} role="tabpanel" aria-labelledby={activeTab}>
