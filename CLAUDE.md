...
- **Version:** 1.13.0 (server `package.json`) — 2026-06-16 Factors per-factor explainer (ROADMAP #2 follow-on): Contribution Waterfall rows are now click-to-expand buttons showing a plain-English factor description + contribution / avg exposure / significance (|t|>1.96) / holding count. `aggregateContributions()` now averages tStat + confidence; reads only data already on each row (no new API, no per-row `symbol` dependency). 966 server + 266 client tests; build clean. On PR #20. See CHANGELOG `[1.13.0]`.
- **Prior:** 1.12.1 (server `package.json`) — 2026-06-16 Backtest running skeleton (`client/src/components/backtest/BacktestRunningSkeleton.tsx`) completes the v1.12.0 polish — mirrors the result layout during `isPending` instead of a blank body — plus an env-schema CI fix (declared the pre-existing-undeclared `SEAL_PRIVATE_KEY` / `EXPLAINER_LATENCY_BUDGET_MS` / `EXPLAINER_TOKEN_CEILING` in `schemas/env-schema.json` so the Schema-Only audit job is green). 966 server + 264 client tests; build clean. On PR #20 (`feat/tax-real-data`). See CHANGELOG `[1.12.1]`.
- **Prior:** 1.12.0 (server `package.json`) — 2026-06-15 Backtest UI polish (ROADMAP Next-candidate #3): replaced the generic pre-run `EmptyState` with `client/src/components/backtest/BacktestEmptyState.tsx`, a preview that ghosts the six real walk-forward metric tiles + the equity curve and points back to the run controls. Client-only. 966 server + 262 client tests; build clean. Backtest is not in the visual-regression suite, so no baseline refresh. NOT yet deployed. See CHANGELOG `[1.12.0]`.
- **Prior:** 1.11.0 (server `package.json`) — 2026-06-15 Factors drill-down (ROADMAP Next-candidate #2): added the Signal Contribution Waterfall (`client/src/components/factors/ContributionWaterfall.tsx`) — a pure, unit-tested `aggregateContributions()` (folds per-(symbol,factor) rows into ranked, net-preserving steps with a top-8 + `Other` bucket) plus a floating-bar waterfall card on the Factors page that shows how each factor builds the net signal. Client-only (no API change). 966 server + 258 client tests; build clean. NOT yet deployed. Visual baseline for the Factors page needs a `test:visual:update` pass. See CHANGELOG `[1.11.0]`.
- **Prior:** 1.10.0 (server `package.json`) — 2026-06-15 Tax page → real data (ROADMAP Next-candidate #4): the Tax Optimization page was wired from a 740-line mock fixture to the live `/api/v1/tax/{report,harvest,wash-sales}` routes. Root gap was an architectural seam — report/harvest/wash-sale ran against the empty in-memory `server.taxLotTracker` and never loaded the persisted `frontier_tax_lots`/`frontier_tax_events`. Added a DB→engine hydration seam (`src/tax/taxRowMapper.ts` pure mapper + `src/tax/loadTrackerFromDb.ts::hydrateTaxTracker`), a public `TaxLotTracker.loadSnapshot()`, and a typed `client/src/api/tax.ts`. `Tax.tsx` now renders real realized activity (mock fixtures retired behind the `?demo=true` link under the `DataSource<T>` contract). 966 server + 250 client tests; build + lint clean; arch.json regenerated (apiModules 9→10). NOT yet deployed. See CHANGELOG `[1.10.0]`.
