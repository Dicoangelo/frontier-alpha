...
**Description:** As an AI agent or human contributor opening the repo for the first time after the v1.2.x wave, I want `ARCHITECTURE.md` and `CLAUDE.md` to accurately describe what's on disk so my mental model matches reality before I touch code.
...
**Files:** `ARCHITECTURE.md`, `CLAUDE.md`, possibly a regenerated `arch-scanner` output
...
- [ ] Reconcile `CLAUDE.md` § "Current State" against `package.json` version and the most recent `git log --oneline -20`
...
- [ ] Verify Supabase migration count (`ls supabase/migrations/ | wc -l`) matches CLAUDE.md
...
- [ ] If using Sentry already (verify `client/src/lib/sentry.ts`): create a Sentry alert rule that fires when `error.count > 50` in 15 minutes. Documented in CLAUDE.md
...
- **FR-9:** ARCHITECTURE.md and CLAUDE.md must accurately reflect the on-disk codebase as of this PRD.
